...
namespace path {
	...
	namespace info {
		...
		/// @return 获取个数
		inline qsizetype getFilesInPath( const QDir &path, QSharedPointer<QVector<QSharedPointer<QFileInfo> > > fileNames ) {
			...
			for( qsizetype index = 0; index < result; ++index )
				fileNames->append(QSharedPointer<QFileInfo>(new QFileInfo(entryInfoList[index])));
			return result;
		}
		...
		/// @param dirNames 目录容器
		/// @return 
		inline qsizetype getDirInPath( const QDir &path, QSharedPointer<QVector<QSharedPointer<QFileInfo> > > dirNames ) {
			QFileInfoList entryInfoList = path.entryInfoList(QDir::AllDirs | QDir::Drives | QDir::NoDotAndDotDot);
			qsizetype result = entryInfoList.length();
			for( qsizetype index = 0; index < result; ++index )
				dirNames->append(QSharedPointer<QFileInfo>(new QFileInfo(entryInfoList[index])));
			return result;
		}
		...
		/// @param dirNames 目录容器
		/// @return 获取数目
		inline qsizetype getPathEntry( const QDir &path, QSharedPointer<QVector<QSharedPointer<QFileInfo> > > fileNames, QSharedPointer<QVector<QSharedPointer<QFileInfo> > > dirNames ) {
			...
			for( qsizetype index = 0; index < result; ++index ) {
				...
				if( fileInfo.isDir() )
					dirNames->append(QSharedPointer<QFileInfo>(new QFileInfo(fileInfo)));
				else if( fileInfo.isFile() )
					fileNames->append(QSharedPointer<QFileInfo>(new QFileInfo(fileInfo)));
			}
			return result;
		}

		/// @brief 获取路径信息，他会自动查看参数，更加安全
		/// @param path 路径
		/// @param fileNames 文件路径容器 
		/// @param dirNames 目录路径
		/// @return 获取个数
		inline qsizetype PROCESSPATH_EXPORT getPathInfo( const QString &path, QSharedPointer<QVector<QSharedPointer<QFileInfo>> > fileNames = nullptr, QSharedPointer<QVector<QSharedPointer<QFileInfo>> > dirNames = nullptr ) {
			qsizetype result = 0;
